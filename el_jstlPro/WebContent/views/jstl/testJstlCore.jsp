...
<h3> * JSTL 선언방법</h3>
	<p>
		JSTL 을 사용 하는 JSP 상단에 page 지시자 아래에 taglib  지시 자를 사용 하여 선언 
		
		prefix: 접두사 . 다른 태그와 구분 할수 있는 name space 를 제공함 
		uri : 실제 웹상의 주소가 아닌, 태그 라이브러리를 나타내는 식별자, 이 지시자를 통해 작성한 태그 이름과 매칭되는 자바코드를 찾음 
	...
	* 변수 출력(c:out)
	...
	<c:out value="${param.name}" default="유재석"/><br>
	<!--param으로 던져준게 없지만 default 설정으로 유재석을 던져줌  -->
	...
	value1의 값은 <c:out value="${value1 }"/>이고,<br>
	value2의 값은 <c:out value="${value2 }"/>이다.
	
	<c:if test="${value1+0 >= value2+0 }">
		<h3>value1 이 더 큽니다 : <c:out value="${value1 }"/></h3>
	</c:if>
	<c:if test="${value1+0 < value2+0 }">
		<h3>value2 이 더 큽니다 : <c:out value="${value2 }"/></h3>
	</c:if>
	<br><br>
	...
	param.no 값은 <c:out value="${param.no}"/>입니다. <br>
